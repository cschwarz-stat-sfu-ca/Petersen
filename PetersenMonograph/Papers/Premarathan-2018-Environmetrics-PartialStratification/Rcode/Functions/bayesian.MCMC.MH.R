...
# this file contains the function "MCMC.MH" - MCMC using Metropolis Hastings
# algorithm for n.updates number of iterations

###############################################################################
###############################################################################
# Function : MCMC.MH - MCMC with Metropolis Hastings algorithm
...
MCMC.MH <- function(n.post.burnin ,n.burn.in,n.thin,nstops,n.chains,
  ...
  # if n.thin = 1 its an array with eliments c(n.post.burnin, n.beta,n.chains)
  thinned.array <- array( 0,  c(((n.post.burnin)/n.thin),n.chains,(n.beta+(3*ncats+3)+length(Data$category))))
  ...
  for(ch in 1:n.chains ) {
    ...
    for( iter in 2:n.updates){
      out <- MH.bayes.update(beta,design,priors,update,Data)
      beta <- out$beta
      ...
      if(iter == (n.burn.in + (kt * n.thin))){
        ...
        reg.parm <- unpack.parm(beta, Data,captureDM,thetaDM,lambdaDM,
                    captureOFFSET,thetaOFFSET,lambdaOFFSET)$full
        # get the categoty totals ( i.e:  category proportion * Population size)
        N.lambda <- reg.parm[(2*ncats+1):(3*ncats) ] * reg.parm[length(reg.parm)]
