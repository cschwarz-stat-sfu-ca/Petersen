---
title: "Petersen Estimator in Fisheries Management"
...
The *Petersen* package can be installed from CRAN in the usual way. (**NOT YET ON CRAN**)
...
-   add sections on equal catchability over time, across groups, model
    averaging etc.
...
from the population. These animals are given an individually numbered
...
likelihood approach used in the *Petersen* package:
...
From the fit, we obtain estimates of overall abundance using the *LP_est()* function 
and the *N_hat* argument. Here there is no stratification
or other groupings of the data, so the formula for *N_hat* is *~1* indicating that we should sum over the entire population.
...
Then this adjusted data is passed to the *Petersen* package:
...
data_one_tag$cap_hist <- paste0(substr(data_one_tag$cap_hist,1,1),substr(data_one_tag$cap_hist,3,3))
data_one_tag[,-(1:2)]
```

We now fit the regular Petersen estimator and get the estimates that are
...
data_one_tag.est.adj <- Petersen::LP_est_adjust(
                             data_one_tag.est$summary$N_hat, data_one_tag.est$summary$N_hat_SE,
...
data_overlook.est.adj <- Petersen::LP_est_adjust(
                             data_overlook.est$summary$N_hat, data_overlook.est$summary$N_hat_SE,
...
ft_aictab(nop.sex.aictab, widths=c("1"=3))
...
ft <- colformat_double(ft, j=c("Estimate","SE","N_hat_LCL","N_hat_UCL"), digits=0)
...
**Check the values in the paragraph below** If only the model where each
...
estimator (@tab-nop-sex-indiv-modavg).
...
#| label: tab-nop-sex-indiv-modavg
...
nop.sumstat.all <- LP_summary_stats(data_NorthernPike)
nop.sumstat.all$length.class <- "xxALL"
...
nop.equal.mf.length.class <- LP_test_equal_mf(data_NorthernPike, "length.class") 
```
...
names(temp) <- make.names(names(temp), unique=TRUE) 
ft <- flextable(temp)
ft <- colformat_double(ft, j=c((nc+1):(2*nc+1)), digits=3)
...
nop.equal.recap.length.class <- LP_test_equal_recap(data_NorthernPike, "length.class") 
```
...
data(data_NorthernPike)
nop.CL <- Petersen::CL_fit(data_NorthernPike, covariate="length")
```
...
nop.CL2 <- Petersen::CL_fit(data_NorthernPike, covariate="length",
...
number of fish passing the at the first sampling station is
...
passing the at the first sampling station is
...
We can compare the fit using AICc in the usual way:
...
LP_modavg(mod..1, mod..2, mod..3, mod..4)
...
Specialized software to fit the above model using the estimating equations has been written in S-Plus and is available upon request. 
...
This can be summarized in a matrix format:
...
BTSPAS.diag.fit1 <- LP_BTSPAS_Diag_fit(
         data_btspas_diag1,
         p_model=~1,
         jump.after=10,
...
```{r}
BTSPAS.diag.est1 <-  LP_BTSPAS_est (BTSPAS.diag.fit1)
...
#| message: false
#| eval: false
BTSPAS.diag.fit3 <- LP_BTSPAS_Diag_fit(
         data_btspas_diag3,
         p_model=~1,
         jump.after=10,
         InitialSeed=234234
...
```{r}
BTSPAS.diag.est3 <-  LP_BTSPAS_est (BTSPAS.diag.fit3)
...
#| eval: false
#| 
BTSPAS.diag.fit2 <- LP_BTSPAS_Diag_fit(
         data_btspas_diag1,
         p_model=~1,
         jump.after=10,
...
BTSPAS.diag.fit4 <- LP_BTSPAS_Diag_fit(
         data_btspas_diag1,
...
```{r}
BTSPAS.diag.est4 <-  LP_BTSPAS_est (BTSPAS.diag.fit4)
...
was seen in the previous example:
...
#| message: false
#| eval: false
BTSPAS.nondiag.fit1 <- LP_BTSPAS_NonDiag_fit( 
         data=data_btspas_nondiag1,
         p_model=~1,
         InitialSeed=34343,
         logitP.fixed=c(23:26), logitP.fixed.values=rep(-10,length(23:26)) #, trace=TRUE
)
...
```{r}
BTSPAS.nondiag.est1 <-  LP_BTSPAS_est (BTSPAS.nondiag.fit1)
...
#| message: false
#| 
kok.est1 <- LP_TL_est(kok.res1, N_hat=~1, conf_level=0.95, trace=FALSE)
kok.est1$summary
...
#| message: false
#| 
twoD.est1 <- LP_TL_est(twoD.res1, N_hat=~1, conf_level=0.95, trace=FALSE)
twoD.est1$summary
...
#| message: false
#| 
twoD.est2 <- LP_TL_est(twoD.res2, N_hat=~1, conf_level=0.95, trace=FALSE)
twoD.est2$summary
...
tag-retention probabilities don't vary over the fish.
...
We now fit a model stratified by sex:
...
The estimate of overall abundance is:
...
Finally, we allow the tag retention probabilities to also vary by sex.
...
Estimates of the tag retention probability for the two sex is
...
overall abundance estimate are:
...
We can now fit the permanent tag models as seen previously:
...
res <- plyr::adply(scenarios[],1, function(x){
   ...
   inner.res <- plyr::ldply(1:x$n.sim, function(inner.sim, x){
     ...
     fit <- Petersen::LP_TL_fit(sim_data, dt_type="notD", p_model=~1, rho_model=~1, trace=FALSE)
...
est.list <- function(modset){
    ...
    estimate <- plyr::laply(modset[ names(modset) != "model.table"], function(x){get.real(x, "N", se=TRUE)$estimate})
    weight   <- modset$model.table$weight
    se       <- plyr::laply(modset[ names(modset) != "model.table"], function(x){get.real(x, "N", se=TRUE)$se})
...
rodli.vgam <- cbind(data_rodli, Petersen::split_cap_hist(data_rodli$cap_hist, make.numeric=TRUE))
...
cat("Confidence intervals for Nhat not readily available??\n")
```
...
rodli.vgam.chapman <- cbind(rodli.chapman, Petersen::split_cap_hist(rodli.chapman$cap_hist, make.numeric=TRUE))
...
AICcmodavg::aictab(list(rodli.vgam.m.0, rodli.vgam.m.t))
...
data(data_NorthernPike)
# split the capture history
NorthernPike.vgam <- cbind(data_NorthernPike, Petersen::split_cap_hist(data_NorthernPike$cap_hist, make.numeric=TRUE))

nop.vgam.time       <- vglm(cbind(t1,t2) ~ 1,   posbernoulli.t, data = NorthernPike.vgam)
nop.vgam.sex.time   <- vglm(cbind(t1,t2) ~ Sex, posbernoulli.t, data = NorthernPike.vgam)
nop.vgam.sex.p.time <- vglm(cbind(t1,t2) ~ Sex, posbernoulli.t(parallel = TRUE ~ Sex), data = NorthernPike.vgam)

# aic table 
library(AICcmodavg)

AICcmodavg::aictab(list(nop.vgam.time, nop.vgam.sex.time, nop.vgam.sex.p.time))
...
nop.vgam.time       <- vglm(cbind(t1,t2) ~ 1,   posbernoulli.t, data = NorthernPike.vgam)
nop.vgam.length.class.time   <- vglm(cbind(t1,t2) ~ length.class, posbernoulli.t, data = NorthernPike.vgam)
nop.vgam.length.class.p.time <- vglm(cbind(t1,t2) ~ length.class, posbernoulli.t(parallel = TRUE ~ length.class), data = NorthernPike.vgam)

# aic table 
library(AICcmodavg)

AICcmodavg::aictab(list(nop.vgam.time, nop.vgam.length.class.time, nop.vgam.length.class.p.time))
```
...
# Appendix D: Brief theory for *BTSPAS* models.
...
-   VGAM methods don't seem to provide ci for N-hat? Is this true?

-   Add S3 method such as coef, summary, AIC, etc so that all the AICmod
    averaging functions work pretty much as expected.

-   Using information about sex ratio 

-   Is it better to pool first and then estimate or to estimate first and
then pool? See Seber (1982, p. 100)
