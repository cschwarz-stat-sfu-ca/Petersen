...
# print the results
# Function :PSkSCR.print.output
# Input : From the function PSkSCR.fit.model
# Output : Print the results

PSkSCR.print.output = function(x){
  ...
  cat("Raw data: \n")
  cat("\n")
  print.data <- data.frame(cbind(x$rawdata$history, x$rawdata$counts))
  ...
  cat("\n")
  cat("Initial probabilities of loss on Capture : \n")
  print(x$initial.unpack.parm$p_loss)
  cat("\n")
  cat("Initial population size for optimization(simple Lincoln Petersen estimator is used) :" ,
      prettyNum(round(x$initial.unpack.parm$N), big.mark = ",") , "\n")
  cat("\n \n")
  ...
  cap.DM = x$captureDM
  rownames(cap.DM)= c(paste(rep( c(paste("p_", c(1:x$st),sep="")), 
                                 each = length(cats)),rep(cats, x$st),sep=""))
  ...
  rownames(x$thetaDM)= c(paste("theta_",c(1:x$st), sep=""))
  ...
  rownames(x$p_lossDM)= c(paste("loss_Time_",c(1:x$st), sep=""))
  print(data.frame(Beta=x$p_lossDM, OFFSET.vector=x$p_lossOFFSET))
  cat("\n")
  ...
  print(x$est$theta)
  cat("\n")
  cat("MLEs for loss on capture probabilities: \n")
  print(x$est$p_loss)
  cat("\n")

  #print number with commas
  cat("MLE for Population size : ", prettyNum(round(x$est$N), big.mark = ",")) 
  
  category.total = prettyNum(round(x$est$N_lambda), big.mark = ",")
  cat(paste("\nMLE for Population size of category", cats,sep = " ", ":",
            category.total))
  ...
  se_p = x$se$p
  colnames(se_p) = c(paste("Time_",c(1:x$st), sep=""))
  ...
  se_theta = x$se$theta
  names(se_theta) =  c(paste("theta_",c(1:x$st), sep=""))
  print(se_theta)
  cat("\n")
  
  cat("SE's of the MLEs of the loss on capture probabilities: \n")
  se_p_loss = x$se$p_loss
  names(se_p_loss) =  c(paste("loss_Time_",c(1:x$st), sep=""))
  print(se_p_loss)
  cat("\n")
  
  cat("SE of the MLE of the population size: ", prettyNum(round(x$se$N),
                                                          big.mark = ","))
  cat(paste("\nSE for Population size of category", cats, sep = " ", ":",
            prettyNum(round(x$se$N_lambda), big.mark = ",")))
  cat("\n")
  
  ######### Table of Histories, observed and expected counts and residuals ####
  cat("\n")
  cat(paste("Observed and Expected counts for capture histories for the model ", 
            x$model.id,sep=" " ,'\n'))
