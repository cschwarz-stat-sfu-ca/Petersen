...
#############################################################################
# Function : PSkSCR.BAYESIAN.plots 
...
PSkSCR.BAYESIAN.plots <- function(Result){
  ...
  st <-  nchar(Result$data$history[1]) # number of sampling occasions
  ...
  plots <- llply((1:(n.beta+ (ncats*st) + ncats + st + st + 1 + ncats)),
  ...
  capture.trace.plots <- do.call(arrangeGrob, 
                                 c(plots[(n.beta+1):(n.beta+ (st*ncats))], 
                                   nrow=ncats,
  ...
  lambda.trace.plots <- do.call(arrangeGrob, 
                                c(plots[(n.beta+(st*ncats) +1):(n.beta+(st*ncats)+ncats)], 
                                  ncol=1,
  ...
  theta.trace.plots <- do.call(arrangeGrob,
                               c(plots[(n.beta+(st*ncats)+ncats+1):(n.beta+(st*ncats)+ncats+st)], 
                                 ncol=1,
                       main=paste("Trace plots for sub-sample Proportions")))
  
  ## trace plots for loss on capture probailities(nu)
  p_loss.trace.plots <- do.call(arrangeGrob,
                               c(plots[(n.beta+(st*ncats)+ncats+st+1):(n.beta+(st*ncats)+ncats+st+st)], 
                                 ncol=1,
                                 main=paste("Trace plots for loss on capture")))
  
  ## trace plots for sub Population size and category total
  pop.trace.plots <- do.call(arrangeGrob, 
                             c(plots[(n.beta+(st*ncats)+ncats+st+st+1):(n.beta+(st*ncats)+ncats+st+st+1+ncats)], 
                               ncol=1,
  ...
  density.plots<-llply((n.beta+1):(n.beta+ (st*ncats)), 
                       function(k){qplot(x=thinned.df[,k],geom="density",
                                         main=paste("Distribution of ",
                                              param_names[k],sep=""))+
                                    xlab(param_names[k]) + 
  ...
  density.plots<-llply((n.beta+ (st*ncats)+1):(n.beta+ (st*ncats)+ncats), 
                       function(k){qplot(x=thinned.df[,k],geom="density",
                                         main=paste("Distribution of ",
                                                    param_names[k],sep=""))+
                                     xlab(param_names[k]) + 
  ...
  density.plots<-llply((n.beta+ (st*ncats)+ncats+1):(n.beta+ (st*ncats)+ncats+st), 
                       function(k){qplot(x=thinned.df[,k],geom="density",
                                         main=paste("Distribution of ",
                                                    param_names[k],sep=""))+
                                     xlab(param_names[k]) + 
  ...
  ## posterior plots for loss on capture probabilities (nu)
  density.plots<-llply((n.beta+ (st*ncats)+ncats+st+1):(n.beta+ (st*ncats)+ncats+st+st), 
                       function(k){qplot(x=thinned.df[,k],geom="density",
                                         main=paste("Distribution of ",
                                                    param_names[k],sep=""))+
                                     xlab(param_names[k]) + 
  ...
  p_loss.posterior.plots <- do.call(arrangeGrob, c(density.plots, ncol=1,
                                                  main=paste("Posterior plots for Loss on Capture Proportions")))
  
  ## posterior plots for Pupulation Size and Category Total
  density.plots<-llply((n.beta+ (st*ncats)+ncats+st+st+1):(n.beta+ (st*ncats)+ncats+st+st+1+ncats), 
                       function(k){qplot(x=thinned.df[,k],geom="density",
                                         main=paste("Distribution of ",
                                                    param_names[k],sep=""))+
                                     xlab(param_names[k]) + 
  ...
  acf.plots <- list()
  for(i in 1:(n.beta+ (st*ncats)+ncats+st+st+1+ncats)){
  ...
  if(ncol(Result$captureDM) > 0){
    capture.acf.plots<- do.call(arrangeGrob, c(acf.plots[(n.beta+1):(n.beta+(st*ncats))],
                                            ncol=2,
  ...
  if(ncol(Result$lambdaDM) > 0){
     lambda.acf.plots<- do.call(arrangeGrob, c(acf.plots[(n.beta+(st*ncats)+1):(n.beta+(st*ncats)+ncats)],
                                               ncol=1,
  ...
  if(ncol(Result$thetaDM) > 0){
    theta.acf.plots<- do.call(arrangeGrob, c(acf.plots[(n.beta+(st*ncats)+ncats+1):(n.beta+(st*ncats)+ncats+st)],
                                              ncol=1,
                      main=paste("Autocorrelation for Sub-Sample Proportions(theta)")))
  }
  
  ## ACF plots for sloss on capture proportions(nu)
  if(ncol(Result$thetaDM) > 0){
    p_loss.acf.plots<- do.call(arrangeGrob, c(acf.plots[(n.beta+(st*ncats)+ncats+st+1):(n.beta+(st*ncats)+ncats+st+st)],
                                             ncol=1,
                                             main=paste("Autocorrelation for Loss on Capture Proportions")))
  }
  
  ## ACF plots for Population size(N) and category total
  pop.acf.plots<- do.call(arrangeGrob, c(acf.plots[(n.beta+(st*ncats)+ncats+st+st+1):(n.beta+(st*ncats)+ncats+st+st+1+ncats)],
                                           ncol=1,
  ...
  res$p_loss.trace.plots <- p_loss.trace.plots
  ...
  res$p_loss.posterior.plots <- p_loss.posterior.plots
  ...
  if(ncol(Result$p_lossDM) > 0){res$p_loss.acf.plots <- p_loss.acf.plots}
...
} # end of the function "PSkSCR.BAYESIAN.plots"
