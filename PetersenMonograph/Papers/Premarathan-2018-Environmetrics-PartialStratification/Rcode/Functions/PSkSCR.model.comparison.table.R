...
PSkSCR.model.comparison.table = function(models=list()){
  ...
  residual.plots <- do.call(arrangeGrob, c(extract.res.plots,nrow = 2,
                                           main=paste("PSkSCR Analysis: Residual Plots with minimum ",
                                                      "AICc values (upto maximum 4 plots)", sep="")))
  
  model_comparison$plots <- residual.plots
  
  return(model_comparison)
} # end of  PSkSCR.model.comparison.table
