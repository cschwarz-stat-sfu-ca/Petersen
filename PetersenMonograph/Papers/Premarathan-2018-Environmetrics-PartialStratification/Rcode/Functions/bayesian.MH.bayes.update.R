...
# this file contains functions for calculations of prior densities and 
# and the function "MH.bayes.update" for Metropolis Hastings algorithm.
...
MH.bayes.update <- function(beta,design,priors,update,Data) {
  ...
  if(ncol(design$p)> 0) {
    for(i in seq_len(ncol(design$p))){
      ...
      log_r_top <-  -(neg.log.likelihood( proposed_beta, Data,captureDM,thetaDM,
                                          lambdaDM,captureOFFSET,thetaOFFSET,
                                          lambdaOFFSET)) + 
                      log.prior.density( proposed_beta[parmindex],
                                            priors$p.mu[i],priors$p.sigma[i]) 
    
      # denominator of the Metropolis Hasting ratio
      log_r_bot <- -(neg.log.likelihood( beta, Data,captureDM,thetaDM,lambdaDM,
